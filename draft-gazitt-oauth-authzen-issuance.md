...
## Policy Decision Point Capabilities
...
Two earlier efforts place an AuthZEN PDP behind an authorization server.
{{I-D.brossard-oauth-rar-authzen}} maps the `authorization_details` of
{{RFC9396}} onto AuthZEN evaluation requests; it has expired, and where this
document treats `authorization_details` as a shaping output it is in scope
for a future revision of that work rather than for this one. The AuthZEN
Working Group's Access Request OAuth Profile addresses the authorization
request rather than the token issuance decision, and the two are
complementary: one governs what a client may ask for, the other whether the
resulting token is issued.

Neither defines a response vocabulary that shapes the issued token, which is
the part of the problem this document is concerned with.
